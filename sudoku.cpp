...
bool solveSudoku(int grid[N][N])
{
    ...
    // Try filling the current location with numbers 1 to 9
    for(int num = 1; num <= 9; num++)
    {
        if(isSafe(grid, row, col, num))
        {
            ...
            // If placing 'num' at the current location doesn't lead to a solution,
            // backtrack and try the next number
            grid[row][col] = 0;
        }
    }

    // No number can be placed at the current location, backtrack
    return false;
}

int main()
{
    int grid[N][N] = {{5, 3, 0, 0, 7, 0, 0, 0, 0}, {6, 0, 0, 1, 9, 5, 0, 0, 0}, {0, 9, 8, 0, 0, 0, 0, 6, 0}, {8, 0, 0, 0, 6, 0, 0, 0, 3},
